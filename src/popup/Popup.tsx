...
export default function Popup() {
  ...
  return (
    <div className="w-100 h-130 p-4 bg-[url('/background.svg')] bg-cover bg-center">
      <div className="m-0.5 bg-white/15 p-2.5 rounded-sm h-full transition-opacity duration-200">
        {(screen === "loading" && <LoadingScreen />) ||
          (screen === "settings" && <Settings setSettings={setSettings} />) ||
          (screen === "main" && (
            <MainScreen settings={settings!} setSettings={setSettings} />
          ))}
      </div>
    </div>
  );
